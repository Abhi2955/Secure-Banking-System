...
public class LoginController {
	...
	public ModelAndView geOtpView(HttpServletRequest request) {
		...
		loginService.sendEmail(username,"Hey "+username+"\n"+ "Your otp is " + Integer.toString(otp)+" for login into Bank of A2Z account."+"\n"+"Have A Nice Day !!."+"\n"+"Happy To Serve You :)."+"\n"+"Bank Of A2Z", "login otp");
	...
	public ModelAndView mailpwd(HttpServletRequest request) {
		...
		try {
			...
			if (user != null && verify) {
				...
				loginService.sendEmail(email, "Hey "+email+","+"\n"+"Your New  password: " + password+"  is for Bank of A2Z."+"\n"+"Have A Nice Day !!."+"\n"+"Happy To Serve You :)."+"\n"+"Bank Of A2Z", "new password");
				message = "Your password was reset. A temporary password was mailed to your email-id";				
			} else
				message = "Username does not exist";
		} catch (IOException e) {
